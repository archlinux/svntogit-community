...
optdepends=('avahi: for network discovery' 'bridge-utils: for briged networking (default)' 'dnsmasq: for NAT/DHCP for guests' 'hal' 'kernel26-ovz24: for openvz guests' 'kvm' 'libsasl: for sasl support' 'lxc' 'openbsd-netcat: for remote management over ssh' 'qemu' 'user-mode-linux' 'virtualbox_bin' 'virtualbox-ose' 'xen')
options=("emptydirs")
...
#  patch -Np1 -i "$srcdir"/unixperms.patch || return 1
  ...
  install -D -m755 "$startdir"/libvirtd.rc.d \
    "$pkgdir"/etc/rc.d/libvirtd || return 1
  install -D -m644 "$startdir"/libvirtd.conf.d \
    "$pkgdir"/etc/conf.d/libvirtd || return 1
}
md5sums=('5f315b0bf20e3964f7657ba1e630cd67'
