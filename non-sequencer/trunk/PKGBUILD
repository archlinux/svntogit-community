...
depends=(cairo gcc-libs glibc hicolor-icon-theme libsigc++ libx11)
makedepends=(jack liblo ntk waf)
options=(debug)
source=($pkgname-$pkgver.tar.gz::https://github.com/linuxaudio/non/archive/refs/tags/$pkgname-v$pkgver.tar.gz)
...
package() {
  depends+=(libjack.so liblo.so libntk.so libntk_images.so)
