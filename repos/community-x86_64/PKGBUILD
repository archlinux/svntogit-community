...
makedepends=(jack ladspa liblo liblrdf libsndfile libxpm ntk waf)
options=(debug)
...
package_non-mixer() {
  ...
  depends=(gcc-libs glibc hicolor-icon-theme libjack.so liblo.so liblrdf.so
...
package_non-timeline() {
  ...
  depends=(cairo gcc-libs glibc hicolor-icon-theme libjack.so liblo.so
  libntk.so libntk_images.so libsndfile.so)
